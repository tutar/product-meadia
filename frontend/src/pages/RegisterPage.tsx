      ...
      <div className="auth-card card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input className="input" type="password" placeholder={t("auth.passwordPlaceholder")} value={password} onChange={e => setPassword(e.target.value)} required minLength={8} />
          </div>
        ...
        <p className="form-footer">{t("auth.hasAccount")} <Link to="/login">{t("auth.signInLink")}</Link></p>
      </div>
    </div>
  );
}
