...
const RUNNING_STATUSES = new Set(["pending", "scripting", "imaging", "video_gen", "compositing"]);
...
export default function TaskWorkspacePage() {
  ...
  const taskList = (items: TaskItem[]) => items.map(task => (
    <button key={task.id} type="button" className={`task-queue-item ${task.id === selectedId && !showCreate ? "is-selected" : ""} ${taskStatusClass(task.status)}`} onClick={() => selectTask(task.id)} aria-current={task.id === selectedId && !showCreate ? "true" : undefined}>
