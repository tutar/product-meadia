...
export interface ModelConfiguration {
  id: string; catalog_model_id: string; provider: string; model_id: string; display_name: string;
  capabilities: ModelStage[]; constraints: Record<string, unknown>; uses_platform_default: boolean;
  verification_status: "unverified" | "verified" | "unavailable" | "revoked";
  verification_error: string | null; verified_at: string | null; revoked_at: string | null;
...
export const modelConfigurationsApi = {
  ...
  create: async (input: { catalog_model_id: string; credential?: string; use_platform_default?: boolean }) => (await api.post<ModelConfiguration>("/model-configurations", input)).data,
  update: async (id: string, input: { credential?: string; use_platform_default?: boolean }) => (await api.patch<ModelConfiguration>(`/model-configurations/${id}`, input)).data,
