...
export const modelConfigurationsApi = {
  ...
  update: async (id: string, input: { display_name?: string; adapter?: string; api_base?: string | null; model_id?: string; capabilities?: ModelStage[]; constraints?: Record<string, unknown>; credential?: string }) => (await api.patch<ModelConfiguration>(`/model-configurations/${id}`, input)).data,
